...
/** 상수
 * 재할당이 금지 된 변수
 * const 키워드에 의해 재할당이 금지되므로 할당 괸 값을 변경할 수 있는 방법은 없다.
 * 일반적으로 상수의 이름은 대문자로 선언하여 상수임을 명확히 나타내며
 * 여러 단어로 이루어진 경우 언더스코어(_)로 구분하여 스네이크케이스로 표현하는 것이 일반적이다.
 */
 ...
 * 변경이 발생하지 않고, 읽기 전용으로 사용하는 원기 값과 객체에는 const 키워드를 사용한다.
