...
//프로토 타입은 프로퍼티를 읽을 때만 사용하며, 프로퍼티를 추가, 수정, 삭제하는 연산은 객체에 직접한다.
...
student.login();
// 메소드느 공유가 되지만, 객체의 상태는 공유되지 않는다.

for(let prop in student){
    console.log(prop);

    // hasOwnProperty : 본인이 가지고 있는 프로퍼티
    // key 에 대응하는 프로퍼티가 상속받은 프로퍼티가 아니고 객체에 직접 구현된 프로퍼티 일 경우에만 true 반환
    student.hasOwnProperty(prop);
