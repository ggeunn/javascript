 ...
 * 빈틈없이 연속적으로 이어져 있어 인덱스를 통해 임의의 요소에 한번에 
 * 접근할 수 있는것을 장점으로 가지고 있다.
 * 
 * 하지만, 자바스크립트의 배열은 일반적인 배열의 동작을 흉내 낸
 * 특수한 객체로 각각의 메모리 공간이 동일한 크기를 갖지 않아도 되고
 * 연속적으로 이어져 있지 않을 수도 있다.
 * 
 * 인덱스를 나타내는 문자열을 프로퍼티키로 가지며
 * length 프로퍼티를 갖는 특수한 객체이다.
 */

/**
 * 프로퍼티 플래그
 * 객체 프로퍼티는 값(value) 와 함꼐 플래그(flag)라 불리는 특별한 속성 3가지를 가진다.
 * writable : true 이면 값을 수정할 수 있다. false 면 읽기만 가능하다.
 * enumerable : true 이면 반복문을 사용해 나열 가능. 그렇지 않으면 반복문 사용 불가.
 * configurable : true 이면 프로퍼티 삭제나 플래그 수정이 가능하다. 그렇지 않으면 프로퍼티 삭제와 플래그 수정은 불가능하다.
 */
